...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    # Common timeout (seconds) to avoid hangs
    try:
        _timeout = float(os.getenv("LLM_REQUEST_TIMEOUT", "60"))
    except Exception:
        _timeout = 60.0
    if model_provider == ModelProvider.GROQ:
        ...
        return ChatGroq(model=model_name, api_key=api_key, timeout=_timeout)
        ...
        return ChatOpenAI(model=model_name, api_key=api_key, base_url=base_url, timeout=_timeout)
        ...
        return ChatAnthropic(model=model_name, api_key=api_key, timeout=_timeout)
        ...
        return ChatDeepSeek(model=model_name, api_key=api_key, timeout=_timeout)
        ...
        return ChatGoogleGenerativeAI(model=model_name, api_key=api_key, timeout=_timeout)
